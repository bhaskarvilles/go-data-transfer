...
func (c *Channels) getQueuedProgress(chid datatransfer.ChannelID) (bool, uint64, uint64, error) {
	chst, err := c.GetByID(context.TODO(), chid)
	if err != nil {
		return false, 0, 0, err
	}
	hasLimit, dataLimit := chst.DataLimit()
	return hasLimit, dataLimit, chst.Queued(), nil
}

func (c *Channels) getReceivedProgress(chid datatransfer.ChannelID) (bool, uint64, uint64, error) {
	chst, err := c.GetByID(context.TODO(), chid)
	if err != nil {
		return false, 0, 0, err
	}
	hasLimit, dataLimit := chst.DataLimit()
	return hasLimit, dataLimit, chst.Received(), nil
}
...
// SetAbleToCompleteTransfer sets the state of whether a data transfer can complete
func (c *Channels) SetAbleToCompleteTransfer(chid datatransfer.ChannelID, revalidateAtCompletion bool) error {
	return c.send(chid, datatransfer.SetAbleToCompleteTransfer, datatransfer.SetAbleToCompleteTransfer)
}
